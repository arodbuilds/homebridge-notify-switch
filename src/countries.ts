/**
 * ISO 3166-1 alpha-2 country code to international dialing code.
 * Used to normalize phone numbers that lack a leading plus sign (SPEC section 10).
 */
...
/** E.164: a plus sign, a non-zero first digit, then up to 14 more digits. */
export const E164_PATTERN = /^\+[1-9]\d{6,14}$/;

export interface NormalizedPhone {
  /** The E.164 value, or undefined when the input could not be normalized. */
  value?: string;
  /** True when the returned value differs from the input. */
  changed: boolean;
}

/**
 * Normalizes a phone number to E.164 using `defaultCountry` when it lacks a leading plus sign.
 * Spaces, dashes, dots and parentheses are removed. A `00` prefix is treated as `+`.
 * For countries outside the North American Numbering Plan a single leading trunk zero is dropped.
 * Returns `value: undefined` if the result is not a valid E.164 number.
 */
export function normalizePhone(input: string, defaultCountry: string): NormalizedPhone {
  ...
  return { value: digits, changed: digits !== raw };
}
