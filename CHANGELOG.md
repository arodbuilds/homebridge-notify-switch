...
A new way to reach people (ntfy), a simpler way to set up a switch, and a security review of the 1.0.1 code. A configuration written by 1.0.x loads unchanged; the settings UI reads your existing switches into the new editor and writes them back exactly as they were.
...
- Harness: the ntfy provider and its card, the security review's log escaping, name rule, forbidden keys, credentials file messages and bounds, the platform default rules and their startup validation, the defaults prompt and Settings dropdown, the switch editor, and the round-trip fixtures under `test/fixtures`.
...
- SPEC.md: sections 3, 5 (new 5.7 Platform defaults, and a note that actions stay the stored form the UI derives), 6, 8, 10, 11.2 (item 8 rewritten for the switch editor, new items 24 and 25), 11.3 and 12 record the above.
