...
- **Switch card in shell order.** Name, then the **Enabled** checkbox, then **Cooldown (seconds)** and **Failure Mode** side by side, then **Failure Sensor** (with its reset field once it is on). **Recipients**, **Extra recipients** and **Send by** are blocks with a bold heading and their explanation directly under it; the extra recipient lists sit two to a row and the Send by checkboxes side by side. The switch's generated ID has moved from the body into **Advanced**, where it is the first field, read-only, captioned "Generated. HomeKit tracks the switch by this id, so you can rename it freely." Under Advanced the order is now the ID, Customize message per channel (with the per-channel messages), the provider dropdowns and the SMS Sender at half width, Priority and Tags on one row, then Hide recipients from each other (BCC). Message, Subject, the variables toggle and the preview line are unchanged (SPEC section 11.2, item 8).
...
- **The summary box is sticky again.** The "Fix these before saving:" box is meant to stay at the bottom of the page while you scroll, but Bootstrap's own alert rule, which the host loads after the plugin's stylesheet, cancelled that and the box scrolled away with the page. The rule now wins, in both themes (SPEC section 11.2, items 15 and 28).
