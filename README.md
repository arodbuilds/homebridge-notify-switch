...
- **Save is disabled**: the list under the Settings section shows what to fix. Every item names the provider, group, or switch it belongs to and the field ("Master switch name is required.", "Account SID is required."), and is a link that moves focus to that field, opening its card's **Advanced** section first if the field is in there. A field shows its error only after you leave it (or jump to it from the list), and a card you just added shows no errors until you leave one of its fields; until then the list reads "Fill in the new provider to enable Save." With more than three items the list collapses to "{n} fields need attention"; click **Show all**.
- **The first scroll over the settings page does nothing, or the page keeps growing**: fixed in 1.3.2. The Homebridge UI applies its own page styles inside the settings page's frame, which used to let the frame scroll on its own before the modal did; the page now leaves all scrolling to the modal. Update the plugin and reopen the settings.
- **"You have unsaved changes from earlier" appears at the top**: you closed the settings without saving last time. **Restore** brings those changes back into the form (every field is checked at once); **Discard** forgets them. The draft is written only once you change something, is kept in your browser for 24 hours, and is cleared once the same configuration has been saved, or when you reset the plugin. It never holds your credentials: a restored provider takes them back from the saved configuration, and a provider you had only just added asks for its secret again.
